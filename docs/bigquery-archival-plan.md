...
3. **Cross-repo coupling.** This repo will encode `paper-prism`'s `runs`/`papers` shape and the summarizer's field names. A change in either surfaces as an archiver failure. The `raw` column caps the blast radius at "field not promoted" rather than "data lost."
