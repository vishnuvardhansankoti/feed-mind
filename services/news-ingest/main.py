"""
main.py — news_ingest Cloud Function entry point.

Fetches the digest feeds, stores what is new in Firestore marked
telegram_status=pending, then rings the notifier's doorbell on
feedmind-telegram-ready. It does not format or send anything itself — that is
feedmind-telegram-notifier's job, and keeping them apart means a Telegram
outage cannot cost us an ingest.
