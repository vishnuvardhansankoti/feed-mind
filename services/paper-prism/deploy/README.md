...
# 1b. Set up the named database the pipeline writes to (create + TTL + index)
#     FIRESTORE_DATABASE defaults to feed-mind-db (see 00-config.sh); override it
#     by exporting FIRESTORE_DATABASE before running.
./01b-setup-firestore-db.sh

# 2. Build (Cloud Build -> linux/amd64) and push the image
./02-build-push.sh

# 3. Configure non-secret env (the interest profiles) and deploy the Job
cp env.yaml.example env.yaml       # edit PROFILE_*, GOOGLE_CLOUD_PROJECT, FIRESTORE_DATABASE
./03-deploy-job.sh
...
| Resource | Name | Notes |
|---|---|---|
| Firestore database | `feed-mind-db` (`FIRESTORE_DATABASE`) | named DB + TTL on `runs`/`run_status` + `runs` composite index (`01b`) |
...
- **Named Firestore database:** the pipeline writes to `FIRESTORE_DATABASE`
  (default `feed-mind-db`), not `(default)`. `01b-setup-firestore-db.sh`
  provisions it (create + TTL + `runs` index). The **web app must read the same
  database** — set `VITE_FIRESTORE_DATABASE` in `web/.env` to match, or the SPA
  queries an empty `(default)`. Collections `runs`/`run_status` are never created
  explicitly; they appear on the job's first write.
